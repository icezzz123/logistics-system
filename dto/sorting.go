...
type SortingScanRequest struct {
	ScanCode  string `json:"scan_code" binding:"omitempty,max=50"` // 订单号或包裹号
	OrderID   uint   `json:"order_id"`                             // 兼容旧调用
	OrderNo   string `json:"order_no" binding:"max=50"`            // 兼容旧调用
	StationID uint   `json:"station_id" binding:"required"`        // 当前站点ID
	TaskID    uint   `json:"task_id"`                              // 兼容旧调用
	TaskCode  string `json:"task_code" binding:"max=50"`           // 任务号
	Remark    string `json:"remark" binding:"max=500"`             // 备注
}
...
type SortingScanResponse struct {
	...
	TaskNo        string                `json:"task_no"`        // 任务号
